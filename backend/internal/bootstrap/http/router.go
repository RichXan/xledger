...
func NewRouterWithDependencies(trustedProxies []string, deps Dependencies) (*gin.Engine, error) {
	...
	if accountingHandler != nil {
		...
		accountingGroup.GET("/transactions/review-summary", accountingHandler.ReviewSummary)
		accountingGroup.GET("/transactions/review-items", accountingHandler.ReviewItems)
	...
	if portabilityHandler != nil {
		...
		portabilityGroup.GET("/import/csv/jobs/:job_id", portabilityHandler.ImportConfirmJobStatus)
		portabilityGroup.GET("/export", portabilityHandler.Export)
		portabilityGroup.POST("/quick-add/preview", portabilityHandler.QuickAddPreview)
		portabilityGroup.POST("/quick-add/confirm", portabilityHandler.QuickAddConfirm)
