...
const variants: Record<ButtonVariant, string> = {
  primary:
    'bg-primary text-white shadow-ambient hover:bg-primary-container active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed',
  secondary:
...
export function Button({
  ...
  return (
    <button
      type={type}
      className={cn(
        'inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition duration-200',
        variants[variant],
        className,
      )}
      {...props}
